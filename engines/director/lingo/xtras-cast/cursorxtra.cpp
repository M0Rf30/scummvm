...
//  24: unk (1, automask?) 28: bitmap cast member
//  32: bitmap cast member 36: frame count (1, or 8 with per-frame
//                             extras following; unparsed)
...
bool parseXtraData(const Common::Array<byte> &data, Info &info) {
	...
	info.member1 = READ_BE_UINT32(&data[28]);
	info.member2 = READ_BE_UINT32(&data[32]);
	info.frameCount = READ_BE_UINT32(&data[36]);

	// member1 names the cursor bitmap and is required; member2 is optional
	if (!info.member1 || info.member1 > 0xFFFF || info.member2 > 0xFFFF)
		return false;
...
CursorXtraCastMember::CursorXtraCastMember(Cast *cast, uint16 castId, XtraCastMember &source)
	...
	if (!_valid)
		...
		debugC(3, kDebugLoading, "CursorXtraCastMember: hotspot: %d,%d, size: %dpx, interval: %dms, members: %d/%d, frames: %d",
				_info.hotspot.x, _info.hotspot.y, _info.sizePx, _info.intervalMs, _info.member1, _info.member2, _info.frameCount);
...
bool CursorXtraCastMember::getCursorInfo(CastMemberID &image, CastMemberID &mask) {
	...
	image = CastMemberID(_info.member1, castLib);
	// The second bitmap reference usually names a sibling member; whether
	// it is a mask or a second animation frame is unverified, so it is
	// only used as a mask when it differs from the image.
	if (_info.member2 && _info.member2 != _info.member1)
		mask = CastMemberID(_info.member2, castLib);
	else
		mask = CastMemberID();
	return true;
}
...
Common::String CursorXtraCastMember::formatInfo() {
	return Common::String::format(
		"cursor Xtra: hotspot: %d,%d, size: %dpx, interval: %dms, members: %d/%d, frames: %d",
		_info.hotspot.x, _info.hotspot.y, _info.sizePx, _info.intervalMs,
		_info.member1, _info.member2, _info.frameCount);
}
